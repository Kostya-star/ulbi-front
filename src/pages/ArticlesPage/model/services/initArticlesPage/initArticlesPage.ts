...
import {
  BIG_VIEW_LIMIT,
  SMALL_VIEW_LIMIT,
} from '../../const/articlesLimit/articlesLimit';
import {
  setLimit,
  setOrder,
  setPage,
...
>('articles/initArticlesPage', async (_, { dispatch }) => {
  const lsView =
    localStorage.getItem(ARTICLES_VIEW_LOCAL_STORAGE) || ArticlesView.SMALL;
  const limit =
    lsView === ArticlesView.SMALL ? SMALL_VIEW_LIMIT : BIG_VIEW_LIMIT;
